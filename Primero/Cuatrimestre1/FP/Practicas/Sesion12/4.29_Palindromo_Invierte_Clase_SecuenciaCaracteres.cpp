...
// Palíndromo e invierte dentro de la clase SecuenciaCaracteres
   ...
   */
   ...
   bool EsPalindromo(){
      ...
      while (izda < dcha && es_palindromo){
         if (Elemento(izda) != Elemento(dcha))
            es_palindromo = false;
         else{
            izda++;
            dcha--;
   ...
   SecuenciaCaracteres Invierte(SecuenciaCaracteres a_invertir){
      int izda = 0,
          dcha = Utilizados() - 1;
      
      while (izda < dcha){
         a_invertir.IntercambiaComponentesDirectamente(izda,dcha);
         izda++;
         dcha--;
      }
...
int main(){
   ...
   inversa = secuencia.Invierte(secuencia);
