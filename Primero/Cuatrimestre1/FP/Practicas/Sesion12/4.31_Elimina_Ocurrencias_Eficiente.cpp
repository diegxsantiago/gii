...
private:
   static const int TAMANIO = 3e6;     // Tamaño necesario para el Quijote
   char v[TAMANIO];
   int util;
...
public:
   ...
   */
   ...
   void EliminaOcurrencias(char a_borrar){
      ...
      escr = lect = PrimeraOcurrencia(a_borrar);

      while (lect < Utilizados()){
         if (Elemento(lect) != a_borrar){
            Modifica(escr,Elemento(lect));
            escr++;
         }
         lect++;
      }
...
int main(){
   ...
   cout << "\n\nCadena resultante: " << secuencia.ToString();
