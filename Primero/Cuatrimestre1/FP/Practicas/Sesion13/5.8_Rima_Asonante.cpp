...
public:
   ...
   */
   ...
   bool EsVocal(int posicion){   
      return Elemento(posicion) == 'a'
             || Elemento(posicion) == 'e'
             || Elemento(posicion) == 'i'
             || Elemento(posicion) == 'o'
             || Elemento(posicion) == 'u'
   ...
   bool RimaAsonante(SecuenciaCaracteres otra_secuencia, int grado_k){
      bool riman = true;
      int i = 0;
      int pos_secuencia1 = Utilizados() - 1,
          pos_secuencia2 = otra_secuencia.Utilizados() - 1;
      
      while (i < grado_k && riman){
         if (EsVocal(pos_secuencia1)){  
            while (! otra_secuencia.EsVocal(pos_secuencia2))
               pos_secuencia2--;

            riman = Elemento(pos_secuencia1) == otra_secuencia.Elemento(pos_secuencia2);
            i++;
            pos_secuencia2--;
         }    
...
int main(){
   ...
   if (una_secuencia.RimaAsonante(otra_secuencia,k))
      cout << "Riman";
   else
      cout << "No riman";
}
